...
// QueryWorklogs chains the current query on the "worklogs" edge.
func (_q *TaskQuery) QueryWorklogs() *WorklogQuery {
	...
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		...
		step := sqlgraph.NewStep(
			...
			sqlgraph.Edge(sqlgraph.O2M, false, task.WorklogsTable, task.WorklogsColumn),
		)
		fromU = sqlgraph.SetNeighbors(_q.driver.Dialect(), step)
		return fromU, nil
	}
...
// the "worklogs" edge. The optional arguments are used to configure the query builder of the edge.
func (_q *TaskQuery) WithWorklogs(opts ...func(*WorklogQuery)) *TaskQuery {
	query := (&WorklogClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *TaskQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Task, error) {
	...
	if _q.withProject != nil || _q.withMilestone != nil || _q.withParent != nil {
		withFKs = true
	}
	if withFKs {
	...
	if query := _q.withWorklogs; query != nil {
		if err := _q.loadWorklogs(ctx, query, nodes,
			func(n *Task) { n.Edges.Worklogs = []*Worklog{} },
			func(n *Task, e *Worklog) { n.Edges.Worklogs = append(n.Edges.Worklogs, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *TaskQuery) loadMilestone(ctx context.Context, query *MilestoneQuery, nodes []*Task, init func(*Task), assign func(*Task, *Milestone)) error {
	...
	nodeids := make(map[uuid.UUID][]*Task)
	for i := range nodes {
		if nodes[i].milestone_tasks == nil {
			continue
		}
		fk := *nodes[i].milestone_tasks
		if _, ok := nodeids[fk]; !ok {
			ids = append(ids, fk)
		}
		nodeids[fk] = append(nodeids[fk], nodes[i])
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "milestone_tasks" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TaskQuery) loadParent(ctx context.Context, query *TaskQuery, nodes []*Task, init func(*Task), assign func(*Task, *Task)) error {
	...
	for _, n := range neighbors {
		nodes, ok := nodeids[n.ID]
		if !ok {
			return fmt.Errorf(`unexpected foreign-key "task_children" returned %v`, n.ID)
		}
		for i := range nodes {
			assign(nodes[i], n)
		}
...
func (_q *TaskQuery) loadChildren(ctx context.Context, query *TaskQuery, nodes []*Task, init func(*Task), assign func(*Task, *Task)) error {
	...
	for _, n := range neighbors {
		fk := n.task_children
		if fk == nil {
			return fmt.Errorf(`foreign-key "task_children" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "task_children" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
}
func (_q *TaskQuery) loadWorklogs(ctx context.Context, query *WorklogQuery, nodes []*Task, init func(*Task), assign func(*Task, *Worklog)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[uuid.UUID]*Task)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	query.Where(predicate.Worklog(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(task.WorklogsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.task_worklogs
		if fk == nil {
			return fmt.Errorf(`foreign-key "task_worklogs" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "task_worklogs" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
